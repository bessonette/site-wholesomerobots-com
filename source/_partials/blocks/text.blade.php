<section class="relative py-16 px-4 sm:px-6">
    <div class="absolute inset-0 bg-[#e5dfd0] mix-blend-multiply pointer-events-none" aria-hidden="true"></div>
    <div class="relative z-10 max-w-2xl mx-auto
                text-stone-700 leading-relaxed font-serif
                [&>p]:mb-5 [&>p]:text-lg
                [&>h2]:text-2xl [&>h2]:font-bold [&>h2]:text-stone-800 [&>h2]:mt-10 [&>h2]:mb-4
                [&>h3]:text-xl [&>h3]:font-semibold [&>h3]:text-stone-800 [&>h3]:mt-8 [&>h3]:mb-3
                [&>ul]:mb-5 [&>ul]:list-disc [&>ul]:pl-6 [&>ul>li]:mb-1
                [&>ol]:mb-5 [&>ol]:list-decimal [&>ol]:pl-6 [&>ol>li]:mb-1
                [&>blockquote]:border-l-4 [&>blockquote]:border-[#b87333]/50 [&>blockquote]:pl-4 [&>blockquote]:text-stone-600 [&>blockquote]:italic [&>blockquote]:my-6
                [&>a]:text-[#5ba8a0] [&>a:hover]:underline">
        {!! $block['content'] ?? '' !!}
    </div>
</section>
